...
int __workshy_main(int argc, char** argv) {
    ...
    for (int i = 0; i < argc; i++) {
        ...
        bool is_bench_rn = strcmp(*argv, "bench") == 0 ||
                           strcmp(*argv, "b") == 0 ||
                           strcmp(*argv, "benchmark") == 0;
        ...
        __workshy_test_function_ptr* test_functions =
            __workshy_get_test_functions();
        char** test_function_names = __workshy_get_test_function_names();

        __workshy_benchmark_function_ptr* benchmark_functions =
            __workshy_get_benchmark_functions();
        char** benchmark_function_names =
            __workshy_get_benchmark_function_names();
        ...
        for (int j = 0; !in_tests && j < *__workshy_get_benchmark_amount();
             j++) {
            if (strcmp(test_function_names[j], argv[i]) != 0) continue;
        ...
        if (in_tests == false && x == -1) {
            fprintf(stderr,
                    "[workshy_warn] Neither benchmark or test could be found "
                    "with the name `%s`",
                    argv[i]);
            ...
            __workshy_run_bench(benchmark_functions[x],
                                benchmark_function_names[x]);
            break;
        }
    }
